...
// Build a map of spec id → site-root-relative HTML path by scanning specs/.
function buildSpecMap() {
  const map = {};
  for (const file of walkMd(path.join(ROOT, 'specs'))) {
    const base = path.basename(file, '.md'); // e.g. "anxiety.spec"
    if (base.endsWith('.spec')) {
      const id = base.slice(0, -5);
      const rel = path.relative(path.join(ROOT, 'specs'), file);
...
// Apply ::tag[] custom extensions to already-rendered HTML.
// depth = number of path segments from SITE_DIR to the current output file.
function applyExtensions(html, specMap, depth) {
  ...
  return html
    ...
    .replace(/::spec-ref\[([^\]]+)\]/g, (_, id) => {
      ...
      const title = href ? `Spec: ${id}` : `Missing spec: ${id}`;
      return `<a class="${cls}" href="${target}" title="${escHtml(title)}">[${escHtml(id)}]</a>`;
    })
...
for (const srcDir of ['drafts', 'outline', 'specs']) {
  ...
  for (const file of walkMd(srcPath)) {
    const rel = path.relative(srcPath, file); // e.g. chapters/01-intro.md
    const outRel = path.join(srcDir, rel.replace(/\.md$/, '.html')); // drafts/chapters/01-intro.html
...
const indexBody = `<h1>Book Publishing Repo</h1>\n<ul>\n${listItems}\n</ul>`;
fs.writeFileSync(
  path.join(SITE_DIR, 'index.html'),
  htmlPage('Book Publishing Repo', indexBody, 0)
);
